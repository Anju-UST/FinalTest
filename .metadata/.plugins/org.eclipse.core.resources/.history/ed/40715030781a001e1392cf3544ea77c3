...
public class LibraryController {
    ...
    @PutMapping("/issue/{id}")
    public ResponseEntity<Issue> updateIssueByUser(@RequestBody Issue uissue, @PathVariable Long id){
    	...
    	Optional<Issue> existingIssue=issueRepository.findByUserAndId(user.get(), uissue.getId());
    	Issue IssuetoUpdate=null;
    	if(existingIssue.isPresent()) {
    		IssuetoUpdate=existingIssue.get();
    		IssuetoUpdate.setIssueDate(uissue.getIssueDate());
    		IssuetoUpdate.setReturnDate(uissue.getReturnDate());
    		IssuetoUpdate.setFine(uissue.getFine());
    		IssuetoUpdate.setPeriod(uissue.getFine());
    		issueRepository.save(IssuetoUpdate);
    		
    	return ResponseEntity.ok().body(IssuetoUpdate);
    	}
    	else
    		return ResponseEntity.noContent().build();
